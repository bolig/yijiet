...
public class TimeUtil {
    ...
    public static long getLateTime(String hhmm) {
        hhmm = getCurrentData() + " " + hhmm;
        SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd HH:mm");
        try {
            Date date = format.parse(hhmm);
            return date.getTime();
    ...
    public static long getTime(String date, String hhmm) {
        hhmm = date + " " + hhmm;
        SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd HH:mm");
        try {
            Date dat = format.parse(hhmm);
            return dat.getTime();
        } catch (ParseException e) {
            e.printStackTrace();
            return -1;
